...
- Ch 1 - Basics in Enriched Category Theory by GM Kelly
** 1
...
#+BEGIN_SRC latex :file diag17.svg
  \begin{diagram}
    ...
    &\rdTo_{r\otimes 1)&&\ldTo{1\otimes \ell)\\
    && X\otimes Y
  \end{diagram}
#+END_SRC
...
\begin{diagram}
(A(z,w)\otimes A(y,z))\otimes
A(x,y)&&\rTo^a&&A(z,w)\otimes(A(y,z)\otimes A(x,y))\\
...
- $V=sSet$ or $Top$
  ...
  2) $T_xy:A(x,y)\rightarrow B(Tx,Ty)$ a map of $\mathscr V$-objects
  subject to
  #+BEGIN_SRC latex :file diag19.svg
    \begin{diagram}
      A(y,z)\otmes A(x,y)&\rTo^{\mu_A}&A(x,z)\\
 ...
 Def: The ordinary category *underlying a $\mathscr{V}-cat$ $\mathscr
 A$ is $A_0=(\mathscr{A})_0=\mathscr{V}-Cat(\mathscr{I},\mathscr(A))$

 $A_0$ is the category with $obj A_0=\obj\mathscr{A}$ and a map $f:XY$
 in $\mathscr A_0$ is just an element $f:I\rightarrow A(X,Y)$.
...
$L^H(\mathscr A,W)$ is a simplicial category s.t $\pi_0^L^H(\mathscr
...
2) For $X,Y$ in $\mathscr C$, the simplicial set $L^H\mathscr{C}(X,Y)$
   ...
   #+BEGIN_SRC latex :file diag20.svg
     \begin{diagram}
       ...
       X&&C_{1,1}&\rLine&C_{1,2}&\rLine\cdots\rLine&&Y\\
       &\rdLine(2,3)&\dTo&&\dTo&&\dTo&\ruLine\\
       &&\dDots&&\dDots&&\dDots\\
