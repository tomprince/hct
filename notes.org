...
Let $\matscr C$ a category, $I$ a =class= of maps in $\mathscr C$. A
map is
...
- $X:\lambda\rightarrow\mathscr C$.
  $\underbrace[X]{X_0\rightarrowX_1\rightarrow\cdots\rightarrow
  X_n\xrightarrow{h_n}\cdots$

#+BEGIN_SRC latex :file diag14.svg
  \begin{diagram}
     C_\beta&\rTo&X_\beta\\
     \dTo^{q_\beta\in I}&&\dTo^{h_\beta}\\
     D_\beta\rTo&\NEpbk X_{\beta+1}\\
   \end{digram}
#+END_SRC
...
**** Smallness
[[file:small-notes.org][Alternative presentation]]
...
$F:\mathscr C\stackrel{\rightarrow\\\leftarrow}\mathscr D:U$.
where $\mathscr C$, $\mathscr C$ are model categories, $F$ and $U$
...
***** Problem
$Cat \stackrel{\xrightarrow{N}\\xleftarrow{\tau}} sSet_*}$ where
...
$I'=\{\text{boundary inclusions }S^{n-1}\rInto D^n}$
$J=\{\text{inclusions }$
...
- $f$ is an =inclusion= if $U$ is open in $X$ iff there exists an
  ...
  - Lemma
    ...
    - Proof
     Let $\lambda$ be an ordianl, $X:\lambda\rightarrow Top$ is a
...
$I\coloneqq\{\text{boundary inclusions $S^{n-1}\rInto D^n | n\ge
0}$ where $S^{-1}=\emptyset$ and $D^n=\{0\}$.
...
$A=\overbrace[h]{X_0\xrightarrow{h_0}X_1\rightarrow\cdots\rightarrow
 X_\beta\rightarrow B}$
Where
#+BEGIN_SRC latex :file diag13.svg
  \begin{diagram}
    C&\rTo&X_\alpha\\
    \dTo^{i\in I}&&\dTo_{h_\alpha}\\
    D&\rTo&\NEpbki X_{\alpha+1}
  \end{diagram}
#+END_SRC
...
- Show that the cofibrant objects in $Top$ are retracts of cell
  complexes?
  - $A$ is a cofibrant if $*\rInto A$ is a cofibration?
#+BEGIN_SRC latex :file diag14.svg
\begin{diagram}
  S^{n-1}&\rTo&*\\
  \dTo&&\dTo\\
