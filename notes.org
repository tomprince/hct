...
- $D(R)(M^\bullet,N^\bullet) = [QM^\bullet, N^\bullet]$
...
- Proof of Thm (Kan's Theorem)
  ...
  3) Classify trivial fibrations
	   $p:X\rightarrow Y$ is a fib iff $p$ has the RLP wrt $\overline I$
...
** Notes for 2011-11-29
- Given a sequence of rings $Q\xrightarrow{\psi}R\xrightarrow{\phi}S we
  have
  $$\Omega_\psi\otimes_RS\rightarrow \Omega_{\phi\psi}\rightarrow
  \Omega_\phi\rightarrow 0$$ is exact.
- Andre-Quillen homology is left derived functor of $\Omega$
- *Example
  - $\phi:R\rightarrowS$ a map of rings ($k$-algebras, $char k =0$
  - we consider the (left-exact) functor $S\otimes_R:Mod-S\rightarrow
    Mod-S$
  - Each exact sequence
    $$0\rightarrow N\rightarrow N'\rightarrow N''\rightarrow 0$$
    gives
    $$S\otimes_R N\xrightarrow{*} S\otimes_R N'\rightarrow S\otimes_R N''\rightarrow 0$$
    $*$ is injective - when $S$ is *flat* as an $R$-module
  - To fix this (when $S$ is not flat) we derived the functor
    $S\otimes_R-$
    1) Construct a projective resolution $F\rightarrow S$ of $S$ as an $R$-module
    2) Show that $F$ is unique up to chain-homotopy of complexes of $R$-modules
    3) Set $T_\ohi=F\otimes_R S$ and $H^\phi_n(N)=H_n(T_\phi\otimes N)$.
       - $H_0^\phi(N)=S\otimes_R N$
       - $H_i^\phi(N)$ for $i>0$ extends * to an long exact sequence
- We want to adapt this process to the functor $\Omega$.
  - Plan: Attach a complex of projective $S$-modules to $\Omega$
    - $\mathscr{L}_\phi$ is cotangent complex
    - $H_0(\mathscr{L}_\phi\otimes N)=\Omega_\phi\otimes_SN$
  - What do we need to do to construct $\mathscr{L}_\phi$
  - We need a suitable category of $R$-algebras, so that our
    projective resolution of $S$ is a complex of projective $S$-modules.
  - This resolution needs to satisfy the following properties
    1) Any resolution of $S$ must reflect the structure of $S$ as an $R$-module
    2) $\mathscr{L}_\phi$ needs to be unique up to homotopy.
    3) $\Omega$ must preserve homotopies
  - First guess of how to do this would be to replace $R$ (a ring) by
    $R$ a dga
    - This doesn't work.
    - Q: Why is the category of dg-$R$-algebras not a good place to
      construct $\mathscr{L}_\phi$?
  - The right category is the category of simplicial $R$-algebras
  - *Definition*
    - Let $R$ be a commutative ring. A simplicial $R$-module is a
      simplicial object in the category of $R$-modules
    - $sR-Mod$
  - Simplicial resolutions
    - We know that *if* $R$-mod is a cofibrantly generated model
      category, then $s(R-mod)$ admits the Reedy model structure.
    - A simplicial resolution of $a\in R-mod$is a cofibrant
      replacement of $A$ viewed as the constant simplicial object in $sR-mod$
  - *Free simplicial extensions* $R$ A commutative ring
    - Let $A$ be a simplicial $R$-algebra, then given a graded set
      $X=\{X_n\}$ of indeterminates, a free extension of $A$ by $X$ is
      a simplicial $R$-algebra $A[X]$ satisfying
      1) $A[X]_n=A_n[X_n]$ polynomial ring of $A_n$ in inderminates x_n$
      2) $s_j(X_n)\subset X_{n+1}$
      3) $A\hookrightarrow A[X]$ is a morphism of simplicial algebras
    - *Example* $S=R[Y]$, $S$ is a free extension, with $x_n=y$
    - How do we know these extensions exists?
      - Given $\phi:A\rightarrow B$ A morphism of simplicial
        $R$-algebras, there exists a simplicial resolution of $B$ over
        $A$ given by a factorization $A\hookrightarrow A[X]\rightarrow B$
      - in the category of simplicial $R$-algebras under $A$
        - this category admits a cofibrantly generated model structure
    - *Remark*
      - Let $S$ be an $R$-algebra and let $M$ be an $R$-module. Let
        $R[X]$ be a simplicial resolution of an $R$-algebra $S$. The
        complex underlying $R[X]$ is an $R$-free resolution of $S$ and
        $\forall n>0$
        $$\pi(R[X]\otimes_R M)=Tor^R_n(S,M)$
  - Resolution of Modules over Rings
    - Let $M$ be an $R$-module.
    - A free resolution of $M$ over $R$ may be built as follows.
    - sequence of free $R$-modules
    - ...
  - We can now define the cotangent complex
    - $\Omega_{-/R}$ is a functor on the category of $R$-algebras
    - $\Omega_\phi=\Omega_{S/R}
    - $S\in sR-alg/R$
    - Why is $\Omega_{-/R}$ a functor?
      - Given a simplicial algebra $S$, we have a simplicial
        $S$-module $(\Omega_{S/R})_n=\Omega_{S_n/R}$
    - Let $\mathscr{L}^\psi\coloneqq \Omega_{A/R}\otimes_AS$
      - simplicial $S$-module
      -
* Notes for 2011-12-02
[2011-12-02 Fri 12:10]
- Q: Why are simplicial algebras better than dgas?
  - *Recall*
    - *Theorem*
      - Let $F:C\leftrightarrow D:G$ and $C$ is a cofibrantly generated
        model category, $I$ and $J$ the generating (acyclic) cofibration.
      - Given $f:x\rightarrow y$ in D$ say that $f$ is a weak
        equivalence/fibration in $D$ if $Gf$ is in $C$.
      - If
        1) $G$ commutes with sequential colimits
        2) $F(i)$, $i$ a cofibration in $C$ has the LLP wrt all
           fibrations in $D$
      - Then $D$ has a cofibrantly generated model structure
        with $FI$ and $FJ$ generation (acyclic cofibrations
    - If the theorem hold, i.e. $D$ has the indicated model structure,
      then $F$ preserves all weak equivalences between cofibrant objects.
  - $DGA_k{\xleftarrow{S}\atop\xrightarrow{\mathscr{O}}Ch_k$ where $S$
    symmetric algebra functor with differential extended by Leibniz rule
  - $Alg_k$ category of commutative $k$-algebras
    - $S_k(A)$ free commutative $k-Alg$ functor extending from $Alg_k$
      to $Ch_k$ doesn't preserve weak-equivalences
  - *Example*- $char_k=2$, $D=DGA_k$ category of non-negatively graded
      commutative dg-$k$-algebras
      $$S:C{\rightarrow\atom\leftarrow:O$$
    - Every object of $C$ is cofbirant when $char k = 2$
    - If we look at $D(n):0\rightarrow k\rightarrow k\rightarrow 0$
      - $D(n)$ is acyclic
      - but $S(0)=k\rightarrow S(D(n))$ is not a weak-equivalence
        - $y\in D(n)_n$ is a non-zero element
        - $y^2\in SD(n)$ is a cycle in degree $n$ which is not a boundary.
  - *Note* If $char_k=0$ then $S$ does preserve weak equivalences
    between cofibrant objects
  - In general, $DGA$ does not inherit a model structure via Kan's theorem
  - From the example
    - The free commutative $k$-algebra functor extending to $Ch_k$
      does not preserve weak-equivalences
    - But the free commutative $k$-algebra functor does preserve
      weak-equivalences between cofibrant objects in $sAlg_k$

** Simplicial $R$-modules (again)
- $R$ commutative ring
- Let $sMod_R$ be the category of simplicial objects in $Mod_R$.
- $X\in SMod_R$, $NX_n=\frac{X_n}{s_0X_{n-1}+\cdots+s_nX_{n-1}}$ with
  $\partial=\sum_0^n(-1)^id:NX_n\rightarrow NX_{n-1}$
- *Theorem* (Quillen)
  - The category $sMod_R$ admits a model category strucutre where a
    morphism $f:X\rightarrow Y$ is
    1) a weak equivalence if $\pi_*X\rightarrow\pi_*Y$ is an iso.
    2) a fibration if $NX_n\rightarrow NY_n$ is onto for $n\ge 1$
    3) a cofibration if the underlying morphism on underlying
       degeneracy diagrams is isomorphic to
       $X_n\rightarrow
       Y_n=X_n\oplus\bigoplus_{\phi:[n]\twoheadrightarrow[m]}\phi^*P_k$
       where $P_k$ is projective as an $R-module$
  - *Lemma* TFAE
    1) $f$ is a fibration of $sMod_R$
    2) $f$ is a fibration of $sSet$
    3) $X\rightarrow \pi_0X\times_{\pi_0Y}Y$ is surjective
- Q: Is the category of simplicial $R$-modules stable?
*** Resolutions of $Mod_r$
- *Definition 1*
  - Given $M\in Mod_R$ a simplicial resolution of $M$ is a cofbirant
    replacement of $M$ in $sMod_r$
- *Definition 2*
  - Let $M\in Mod_R$. Then a simplicial resolution for $M$ is an
    augmented simplicial $R$-module $X\rightarrow M$ s.t.
    1) $X_n$ is a projective $R$-module for $n\ge0$
    2) The augmentation map $c:X\rightarrow M$ induces an isomorphism
       of $R$-modules
       $$\pi_kX=\begin{cases} M&\text{if }k=0\\0&\text{if }k\ne0\end{cases}$$
- Given $X$ a simplicial object in any category $C$, an augmentation
  is a morphism $d_0X\rightarrow A$,
  s.t. $d_0d_0=d_0d_1:X_1\rightarrow A$
  - *Exercises/Note* If $X$ is a simplicial $R$-module an augmentation
    induces a map $\pi_0X\righarrow A$
- *Lemma*
  - If $X\rightarrow M$ is an augmented $R$-module, then
    $NX\rightarrow M$ is an augmetned chain complex
  - If $X\rightarrow M$ is a simplical resolution of $M$ then
    $NX\rightarrow M$ is a projective resolution of $M$.
- $Alg_R$ category of commutative $R$-algerbras
  - The obvious choice for a class of projective objects are the free
    commutative algebras $S_R(P)$ on projective $R$-algberas $P$.
  - *Example* $P=R^n$ with basis $\{x_i\}
    - $S_R(P)=R[x_i]$
- *Goal* To build resolutions of commutative $R$-algebras in $sMod_R$
  instead of $Ch$
- In the category $sAlg_r$, every $R$-algebra $Y$ has a natural
  augmentation over $\pi_0Y$
  - $\pi_0Y=Y_0/(Im(d_0-d_1))$
  - all $d_i$'s in $sAlg_k$ are algebra maps, and $d_{i+1}s_i=d_is_i=1$.
- *Definition* of a $A\in Alg_R$ is an augmented simplicial
  $R$-algebra st
  1) there exists a functor $P:\Delta^{op}_+\rightarrow Mod_R$ s.t.
     $P_n$ is projective as an $R$-module and $X_+\cong S_R(P)$ is an
     isomorphism in $Alg_R$
  2) The augmentation induces an isomorphism
     $$\pi_kX=\begin{cases} A&\text{if }k=0\\0&\text{if}k\ne0\end{cases}$$
     of $R$-algebras
- *Theorem* $S_r:sMod_R\rightleftarrows sAlg_R:\mathscr{O}$ is a
  Quillen adjunction.
  - *Point* $S_R$ preserves weak equivalence between cofibrant objects

- Rezk - Every category of simplicial algebras has a left proper model structure
  - Best proof of model structure on $sAlg_R$
- Let $X$ be an $R$-algebra, and $\Omega_{X/R= I/I^2$ with
  $$\partial:X\rightarrow \Omega_{X/R}:g\mapsto [(y\otimes 1-1\otimes y)]$$
  - We have a natural map
    $$Mod_X(\Omega_{X/R}, M)=Hom_X(\Omega_{X/R}\rightarrow Der_R(X,M)$$
- Let $A$ be an $R$-algebra and let $Alg_R/A$ be the category of
  commutative $R$-algebras over $A$
- We have a functor $ab(A)=A\ltimes: Mod_R\rightarrow Alg_R/A$
  - $M\in Mod_R$, $A\ltimes M=A\oplus M$ with multiplication
    $$(a,x)\cdot(b,y)=(ab,ay+bx)$$
- *Facts*
  - $Alg_R/A(X,A\ltimes M)=Der_R(X,M)$
  - $Der_(X,M)=Mod_A(A\otimes\Omega_{X/R},M)$
- *Prop* The functor $X\maspto A\otimes_X\Omega_{X/R}$ is left adjoint
  to the functor $M\maspto A\ltimes M$
- $M\mapsto A\ltimes M$ extends levelwise (prolongs) to simplicial algebras.
  - $sMod_A\mapsto sAlg_R/A$
  - preserves weak equivalences and fibrations
  - i.e. we have a Quillen adjunction $\Omega_{-/R}:sAlg_R/A\rightleftarrows sMod_R:A\ltimes-$
- $L_{A/R}=A\otimes_X\Omega_{X/R}$ the cotangent complex
  - is used to construct the derived functor of $A\ltimes-$ when
    $X\rightarrow A$ is a cofibrant resolution of $A$ in $sAlg_R/A$
  - $\pi_nL_{A/R}=H_qNL_{A/R}$ André-Quillen homology
* Notes for 2011-12-06
- $L_{A/R}$ a functor of arrows $R\rightarrow A$ or a functor of pairs $(A,R)$
- $B\otimes_A L_{A/R}\rightarrow L_{B/S}$
-
  #+BEGIN_SRC latex :file diagram43.svg
    \begin{diagram}
      X&\rTo&S\otimes_RX&\rInto^i& Y\\
      \dOnto^p&&\dTo&&\dTo^q\\
      A&\rTo&B&\rTo&B
    \end{diagram}
  #+END_SRC
** Toplogical André-Quillen Homology
- André-Quillen homology of a commutative ring spectrum
- "the derived version of André-Quillen"

- $Sp^\Sigma$ - $\mathbb{S}$-modules
- Ring spectrum is 1) monoid in $Sp^\Sigma$ 2) $\mathbb{S}$-algebra
- Commutative ring spectrum is a commutative monoid in $Sp^\Sigma$
  ($E_infty$-ring spectrum)
- Let $R$ be a commutative ring spectrum
  - $\pi^s_*$ is a graded commutative ring
  - modules over $R$ ($Mod_R$)
    - $\pi^s_*M$ is graded module over $\pi^s_*R$
- TAQ (Basterra)
  - $R$ a fixed commutive ring spectrum
  - $Alg_R$ category of commutative $R$-algebras
  - $R\ltimes M:Alg_R/R\rightarrow Mod_R$
  - The changes for ring spectra include

    1) $R$ is connective
       - *Definition* A symmetric spectra is *connective* if
         $\pi_nX=0$ when $n<0$
    2) We have to restrict to connective, non-unital ring spectra
       - $I_R:Alg_R/R\rightarrow Rng$ we define an augmentation functor
         #+BEGIN_SRC latex :file diagram44.svg
           \begin{diagram}
             I_R(A)\wedge_R I_RA && & \rTo& A\wedge_R A \\
             &\dTo(2,4)\dTo && && \dTo\\
             &&I_R(A)\rTo& A\\
             && \dTo && \dTo\\
             && * &\rTo& R\
           \end{diagram}
         #+END_SRC
       - $I_R(A)\wedge_R I_R(A)\rightarrow I_R(A)$ is unique!
         - therefore $I_R(A)$ has a connective non-unital $R$-algebra structure
       - $\Omega_{A/R} =^? I/I^2$
    3) We have $Ab^\Delta_R=A\ltimes M: Alg_R/A\rightarrow Mod_A$ for
       simplicial algebras
       - We want this same functor on ring spectra (replace abelian
         with abelian up to homotopy):
         $$ Ab_R^A=A \ltimes: Ho(Alg_{R/A})\rightarrow Ho(Mod_A) $$
       - Take a derived functor of
         $$Ab^A_R(B)=A\ltimes B \coloneqq LO_B RI_B(p_*B\wedge_R A)$$
         where $p$ is cofibrant replace and $Q$ is $I$'s left adjoint
         - $I_R:Alg_{R/A}\leftrightarrow Rng:Q_A$ ($Q_A$ is square 0 extension)
       - Now we can define
         $$\Omega_{A/R}\coloneqq Ab^A_R(A)=LQ_A RI_A(p_*A\wedge_R A)$$
         where $Mod_A$ is category of modules over $A$ a connective,
         non-untial ring spectrum
  - $TAQ_*(A,R; M) = \pi_*(\Omega_{A/R}\wedge A M)$
    - we replace augmentation ideal with the augmentation functor (given
      by a pullback)
    - we replace $I/I^2$ with the unit of an adjunction
  - $R$ a commutative ring, $A$ a commuative $R$-algebra
    - Q: Can we compare $AQ(A,R,M)$ with $TAQ(HA, HR, HM)$ where $HA$
      is the Eilenberg-MacLane ring spectrum associated to $A$.
    - Given a simplicial ring $A$, how do we apply $H(-)$ to $A$ in
      order to get a simplicial object int he category of ring spectra?
      - $\pi_*A=\pi_*HA_*$ just apply $H$ to each $A_n$, giving a
        simplicial object in ring spectrum
    - Guess: $AQ_*(A,R;M)=\pi_*\mathscr{L}_{A/R}$ shuod be related (or
      the same as) $TAQ_*(HA,HR;HM)=\pi_(\Omega_{A/R}\wedge_A M)=\pi_*(\mathscr{L}_{A/R})$
      - These are not the same.
    - Why?
      - André-Quillen detects very sensitive info about morphisms of
        commutative rings
        - Let $A$ be a finitely generated $R$-algebra
        - $R\rightarrow A$ is smooth iff $\mathscr{L}_{A/R}\rightarrow
          \Omega_{A/R}$ is a weak equivalence and $\Omega_{A/R}$ is a
          projective $A$-module
        - $R\rightarrow A$ is étale iff $H_*\mathscr{L}_{A/R}$ vanishes
      - Note: AQ Tells us about maps of rings, TAQ tells us about
        maps of spectra.
      - Over a field $k$, $char k=0$, there is a spectral sequence
        $$E^2_{p,q}=AQ_p(A;k;M)\otimes TAQ_q(Hk[x],Hk;HM) \Rightarrow TAQ_{p+q}(HA,Hk;HM)$$
      - $\Gamma$-homology (of $H\mathbb{Z}$) (Robinson) and AQ homology agree for flat
        extensions of rings
    - Free commutative algebra functor
      - $O:DGA_R\leftrightarrow Ch_k:S_k(-)$
      - $Omeags_{X/A}=P_*X\wedge_A X$ (where $X$ is a cofibrant $A$-module)
      - McCarthy: $P_*X=$ converging Goodwillie tower of the forgetful
        functor $Alg/R\rightarrow Mod_R$
      - Dugger-Shipley: have an obstruction theory to having a $P_{*A}X$
      - Mandel-Basterra: to talk about $\Omega_{P_*X/A}$ where $A=BP$.
- [[math.uic.edu/~bshipley][Summer School 2001 - Iyengar]]
- Paul Goerss and Kristen Schemmerhorn
* Notes for 2011-12-09
- [[http://www.math.uwo.ca/~plowrey/teaching/M9502B/][Math 9502 - Topics in Homotopy Theory: Derived Algebraic Geometry]]
- Eric Finster (Lausanne) - Lecture notes - intro to Quasicatgories
- For Next Term
  - Definition of model cat, cofibrantly generated, (total) dervied
    functors, Quillen pairs and examples
    - Dwyer-Splinski
    - Chain Complexes (bounded and unbounded)
      - perfect complexes
      - stable model categories
  - From stable homotopy theory
    - how to tell if you have a stable model category
    - Therorem (Schwede-Shipley)
      - If you have a set of compact generators
  - Symmetric Spectra
    - Definition
    - Examples
    - Suspention/loops, Free, Endomorpism, internal hom
    - Schwede (book)
  -
    - Dervied Mapping Spaces $Map^h(X,Y)$
    - is a Quasicategory
    - algebraic geometry from "functor of points" point-of-view
    - $\mathbb{R}Hom(Spec R, X)=Map^h(Spec R, X)$
    - Model Categories and Simplicial Methods - Goerss-Schemmerhorn
  - (Framings & Coframings - Hovey ch.5), (Toen-Vezzzosi)
